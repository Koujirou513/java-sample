...
public class Sales {
    ...
    private LocalDate targetDate; // 販売日
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "target_date", referencedColumnName = "target_date", insertable = false, updatable = false)
    @ToString.Exclude  // 無限ループを防ぐために除外
    private SalesWeather salesWeather; // 天気情報
    ...
    // カスタムコンストラクタ
    public Sales(LocalDate targetDate, User createdBy) {
